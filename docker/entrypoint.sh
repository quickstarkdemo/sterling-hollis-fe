...
replace_env_vars() {
  ...
  for file in /usr/share/nginx/html/assets/*.js /usr/share/nginx/html/index.html; do
    if [ -f "$file" ]; then
      ...
      sed -i "s|__VITE_API_PROXY_TARGET__|${VITE_API_PROXY_TARGET:-https://sterling-hollis-be.quickstark.com}|g" "$file"
      ...
      sed -i "s|__VITE_DEMO_OBSERVABILITY_UI__|${VITE_DEMO_OBSERVABILITY_UI:-false}|g" "$file"
    fi
  done

  sed -i "s|__VITE_API_PROXY_TARGET__|${VITE_API_PROXY_TARGET:-https://sterling-hollis-be.quickstark.com}|g" /etc/nginx/conf.d/default.conf
}
