    ...
    <Box className="trace-replay-bar active" aria-label="Trace replay controls">
      ...
      <HStack gap={3} className="trace-replay-scrubber">
        ...
        <HStack gap={1} role="group" aria-label="Replay speed">
          {SPEEDS.map((speed) => (
            <button type="button" key={speed} className={replay.speed === speed ? "trace-speed active" : "trace-speed"} aria-pressed={replay.speed === speed} onClick={() => replay.setSpeed(speed)}>{speed}×</button>
          ))}
        </HStack>
      </HStack>
    </Box>
